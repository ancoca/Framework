...
public class pagina_admin {
    ...
    public static void inicializa() {
        int rowCount = 0;
        rowCount = ((miniSimpleTableModel_admin)List_admin.TABLA.getModel()).getRowCount();
        
        int v = rowCount%itemsPerPage==0 ? 0 : 1;
        maxPageIndex = rowCount/itemsPerPage + v;
        
        box.setBorder(BorderFactory.createEmptyBorder(2,2,2,2));
        List_admin.jPanel4.setLayout(new BorderLayout());
        List_admin.jPanel4.add(pagina_admin.box);  
    }
    ...
    public static void initLinkBox() {
        ...
        if(  (rowCount<=itemsPerPage) && (rowCount>0)  ){ //caben todos los datos en la misma página
            ...
            List_admin.primero.setEnabled(false);
            List_admin.ANTERIOR.setEnabled(false);
            List_admin.SIGUIENTE.setEnabled(false);
            List_admin.ultimo.setEnabled(false);
            List_admin.CAJA.setText("");
            ...
            List_admin.primero.setEnabled(currentPageIndex>1);
            List_admin.ANTERIOR.setEnabled(currentPageIndex>1);
            List_admin.SIGUIENTE.setEnabled(currentPageIndex<maxPageIndex);
            List_admin.ultimo.setEnabled(currentPageIndex<maxPageIndex);
            List_admin.CAJA.setText(Integer.toString(currentPageIndex) + String.format(" / %d", maxPageIndex));
